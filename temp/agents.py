...
class DQNAgent:
    def __init__(self, env, replay_buffer, target_update_freq=TARGET_UPDATE_FREQ,
        ...
        self.noutputs = env.action_space.n if self.phase == "size" else 4  # Chỉnh sửa [change]: n_pos_outputs -> noutputs
    ...
    def select_action(self, state):
        ...
        if random.random() <= self.epsilon:
            ...
            with torch.no_grad():
                state = torch.from_numpy(state.astype(np.float32)).float().unsqueeze(0).to(device) # [change]: add .astype(np.float32) to state to check type of state
                if self.phase == "center":
