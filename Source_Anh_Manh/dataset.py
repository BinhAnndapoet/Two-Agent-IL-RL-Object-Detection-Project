...
class PascalVOCDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[str, torch.Tensor, List[List[int]], List[str]]:
        ...
        img_np = np.array(img)
        if img_np.ndim == 2:  # Handle grayscale images # [change]: .shape.nd -> .ndim
            img_np = cv2.cvtColor(img_np, cv2.COLOR_GRAY2RGB)
        
        # Resize image to 448x448
        img_np = cv2.resize(img_np, TARGET_SIZE).astype(np.float32)
        # img_np = img_np / 255.0  # Normalize to [0,1] # [change]: not needed as transforms will handle it
        ...
        img_tensor = torch.zeros((3, TARGET_SIZE[1], TARGET_SIZE[0]))  # Default fallback
        # Apply transform
        if self.transform:
            img_tensor = self.transform(img_np) # [C, H, W]
...
def custom_collate_fn(batch: List[Tuple[str, torch.Tensor, List[List[int]], List[str]]]) -> Tuple[List[str], torch.Tensor, List[List[List[int]]], List[List[str]]]:
    """
...
def load_pascal_voc(root_dir: str, il_ratio: float = 0.2) -> Tuple[dict, List[str]]:
    """
    ...
    # Load trainval dataset for training (IL + DQN)
    trainval_dataset = PascalVOCDataset(root_dir, split='trainval')
    
    # Split trainval into IL (20%) and DQN (80%) according to proposal
    indices = trainval_dataset.indices
    img_ids = trainval_dataset.img_ids
    train_il_indices, train_dqn_indices = train_test_split(
        indices, train_size=il_ratio, random_state=42
    )
    
    # Load separate val dataset for testing (true out-of-sample test)
    test_dataset = PascalVOCDataset(root_dir, split='val')
    
    # Create dataset instances
    transform = trainval_dataset.transform
    train_il_dataset = PascalVOCDataset(root_dir, split='trainval', transform=transform)
    train_dqn_dataset = PascalVOCDataset(root_dir, split='trainval', transform=transform)
    
    # Assign indices and img_ids for training splits
    ...
    # Create DataLoaders
    train_il_loader = DataLoader(train_il_dataset, batch_size=32, shuffle=True, num_workers=0, collate_fn=custom_collate_fn)
    train_dqn_loader = DataLoader(train_dqn_dataset, batch_size=32, shuffle=True, num_workers=0, collate_fn=custom_collate_fn)
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=0, collate_fn=custom_collate_fn)
    
    # Create dataset splits following ML best practice
    datasets = {
        'train_il': (train_il_dataset, train_il_loader),            # 20% of trainval for IL
        'train_dqn': (train_dqn_dataset, train_dqn_loader),         # 80% of trainval for DQN  
        'test': (test_dataset, test_loader)                         # # Separate val split for testing
    }
    
    print(f"Dataset splits created:")
    print(f"  - Train IL: {len(train_il_dataset)} samples ({len(train_il_dataset)/len(trainval_dataset)*100:.1f}% of trainval)")
    print(f"  - Train DQN: {len(train_dqn_dataset)} samples ({len(train_dqn_dataset)/len(trainval_dataset)*100:.1f}% of trainval)")
    print(f"  - Test: {len(test_dataset)} samples (separate val split)")
    
    return datasets, trainval_dataset.class_names

def get_env_config(root_dir: str) -> dict:
    """
