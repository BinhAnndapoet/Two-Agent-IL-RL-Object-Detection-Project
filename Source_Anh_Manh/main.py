...
def main():
    ...
    if center_il_model is not None:
        try:
            ...
            print("Successfully loaded center IL model weights into DQN")
        except RuntimeError as e:
            print(f"Warning: Could not load center IL model weights due to architecture mismatch: {e}")
            print("Center agent will start with random initialization")
    ...
    if size_il_model is not None:
        try:
            ...
            print("Successfully loaded size IL model weights into DQN")
        except RuntimeError as e:
            print(f"Warning: Could not load size IL model weights due to architecture mismatch: {e}")
            print("Size agent will start with random initialization")
    ...
    # Evaluate on test set (separate VOC2012 val split)
    print("Evaluating on test set...")
    env_config['phase'] = 'test'
    env_config['env_mode'] = 1  # [Sửa đổi]: Bật evaluation mode để lưu ground truth
    env = DetectionEnv(env_config) 
    ...
    for image_ids, image_batch, gt_bbox_batches, gt_label_batches in test_loader:
        ...
        env.current_gt_bboxes = gt_boxes                                        # Changed variable name
        env.current_gt_labels = gt_labels                                       # Changed variable name
        env.reset()
