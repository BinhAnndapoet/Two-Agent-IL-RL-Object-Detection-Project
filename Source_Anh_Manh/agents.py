...
class DQNAgent:
    def __init__(self, env, replay_buffer, target_update_freq=TARGET_UPDATE_FREQ,
        ...
        # [FIX]: Force environment to match agent phase BEFORE getting state
        ...
        # [FIX]: Use env.number_actions for extensibility according to Proposal
        if self.phase == "center":
            # Center: number_actions (6) + n_classes according to Proposal
            self.noutputs = env.number_actions + self.n_classes
        else:
            # Size: number_actions (6) according to Proposal
            self.noutputs = env.number_actions
            
        # Verification: Ensure consistency with action_space
        assert self.noutputs == env.action_space.n, f"Action space mismatch: {self.noutputs} != {env.action_space.n}"
        
        # [FIX]: Restore original environment phase
        ...
        self.episode_info: Dict[str, Any] = {
            ...
            "avg_recall": [],
            "mAP": [],
            "best_episode": {"episode": 0, "avg_reward": float('-inf')},
            "solved": False,
            "eps_duration": 0
        }
    ...
    def select_action(self, state):
        ...
        if random.random() <= self.epsilon:
            ...
            with torch.no_grad():
                state = torch.from_numpy(state.astype(np.float32)).float().unsqueeze(0).to(device)
                if self.phase == "center":
    ...
    def expert_agent_action_selection(self, use_ground_truth=False, target_bbox=None):
        ...
        for action in range(self.noutputs):
            ...
            # [FIX]: Use env.number_actions instead of hardcoded 6
            if self.phase == "center" and action >= self.env.number_actions:
                if reward > 0:
    ...
    def update(self):
        ...
        if self.phase == "center":
            ...
            with torch.no_grad():
                ...
                # [FIX]: Remove unused class_target
            ...
            pos_mask = actions < 4
            # [FIX]: Use env.number_actions instead of hardcoded 6 for class mask
            class_mask = (actions >= self.env.number_actions) & (actions < self.env.number_actions + self.n_classes)
            conf_mask = actions == 5
            done_mask = actions == 4
            ...
            # [FIX]: Adjust class target computation using env.number_actions
            class_loss = self.criterion_class(class_q[class_mask], (actions[class_mask] - self.env.number_actions).squeeze()) if class_mask.any() else torch.tensor(0.0, device=device)
            conf_loss = self.criterion_pos(conf_q[conf_mask], conf_target[conf_mask]) if conf_mask.any() else torch.tensor(0.0, device=device)
            # [FIX]: Use done_mask instead of conf_mask for done_loss condition
        ...
        loss.backward()
        # [FIX]: Add None check for gradient clipping
    ...
    def save(self, path="models/dqn"):
        ...
        # [MỚI] - Use pickle instead of numpy for dict
        with open(f"{path}_episode_info.pkl", 'wb') as f:
            pickle.dump(dict(self.episode_info), f)
